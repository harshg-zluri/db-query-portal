...
describe('RequestController', () => {
    ...
    describe('create', () => {
        ...
        it('should create a valid SQL request', async () => {
            ...
            (QueryRequestModel.countPendingRequestsByUser as jest.Mock).mockResolvedValue(0);
            (QueryRequestModel.createRequest as jest.Mock).mockResolvedValue({ id: 'req-1', ...validBody });

            await RequestController.create(req as Request, res as Response, next);

            expect(QueryRequestModel.createRequest).toHaveBeenCalled();
        ...
        it('should throw ValidationError if db type mismatch', async () => {
            req.body = validBody;
            (DatabaseInstanceModel.findInstanceById as jest.Mock).mockResolvedValue({ ...mockInstance, type: 'mongodb' });
        ...
        it('should throw ValidationError if database not in instance', async () => {
            req.body = validBody;
            (DatabaseInstanceModel.findInstanceById as jest.Mock).mockResolvedValue({ ...mockInstance, databases: ['other'] });
        ...
        it('should return 429 if limit exceeded', async () => {
            ...
            (DatabaseInstanceModel.findInstanceById as jest.Mock).mockResolvedValue(mockInstance);
            (PodModel.findPodById as jest.Mock).mockResolvedValue(mockPod);
            (QueryRequestModel.countPendingRequestsByUser as jest.Mock).mockResolvedValue(10); // Limit is 5

            await RequestController.create(req as Request, res as Response, next);

            expect(res.status).toHaveBeenCalledWith(429);
            expect(QueryRequestModel.createRequest).not.toHaveBeenCalled();
        });
        ...
        it('should handle script upload', async () => {
            ...
            (PodModel.findPodById as jest.Mock).mockResolvedValue(mockPod);
            (QueryRequestModel.countPendingRequestsByUser as jest.Mock).mockResolvedValue(0);
            (QueryRequestModel.createRequest as jest.Mock).mockResolvedValue({ id: 'req-1' });

            await RequestController.create(req as Request, res as Response, next);

            expect(QueryRequestModel.createRequest).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should validate against safety fallback if discovery fails', async () => {
            ...
            (DatabaseInstanceModel.findInstanceById as jest.Mock).mockResolvedValue(emptyInstance);
    ...
    describe('getMyRequests', () => {
        it('should list user requests', async () => {
            ...
            (QueryRequestModel.findRequestsByUserId as jest.Mock).mockResolvedValue({ requests: [], total: 0 });

            await RequestController.getMyRequests(req as Request, res as Response, next);

            expect(QueryRequestModel.findRequestsByUserId).toHaveBeenCalledWith('user-1', 1, 10, undefined);
        ...
        it('should filter by status', async () => {
            ...
            expect(QueryRequestModel.findRequestsByUserId).toHaveBeenCalledWith('user-1', 1, 20, 'pending');
        });
        ...
        it('should handle errors', async () => {
            ...
            (QueryRequestModel.findRequestsByUserId as jest.Mock).mockRejectedValue(error);
    ...
    describe('withdraw', () => {
        it('should throw NotFoundError if request not found during withdraw check', async () => {
            req.params = { id: 'req-1' };
            (QueryRequestModel.withdrawRequest as jest.Mock).mockResolvedValue(null);
        ...
        it('should withdraw request successfully', async () => {
            req.params = { id: 'req-1' };
            (QueryRequestModel.withdrawRequest as jest.Mock).mockResolvedValue({ id: 'req-1', status: 'withdrawn' });

            await RequestController.withdraw(req as Request, res as Response, next);

            expect(QueryRequestModel.withdrawRequest).toHaveBeenCalledWith('req-1', 'user-1');
            expect(res.status).toHaveBeenCalledWith(200);
        });

        it('should throw specific errors if withdraw fails', async () => {
            req.params = { id: 'req-1' };
            (QueryRequestModel.withdrawRequest as jest.Mock).mockResolvedValue(null); // Fail first try
            (QueryRequestModel.findRequestById as jest.Mock).mockResolvedValue({
                id: 'req-1',
                userId: 'user-1',
    ...
    describe('getById', () => {
        ...
        it('should handle errors', async () => {
            const error = new Error('Database error');
            (QueryRequestModel.findRequestById as jest.Mock).mockRejectedValue(error);
    ...
    describe('list', () => {
        ...
        it('should allow managers', async () => {
            req.user = { ...mockUser, role: UserRole.MANAGER, managedPodIds: ['pod-1'] };
            (QueryRequestModel.findRequestsWithFilters as jest.Mock).mockResolvedValue({ requests: [], total: 0 });
    ...
    describe('downloadResult', () => {
        ...
        it('should allow owner to download', async () => {
            ...
            (QueryRequestModel.findRequestById as jest.Mock).mockResolvedValue(mockRequestWithResult);
        ...
        it('should throw NotFoundError if result missing', async () => {
            req.params = { id: 'req-1' };
            (QueryRequestModel.findRequestById as jest.Mock).mockResolvedValue({ ...mockRequestWithResult, executionResult: null });
